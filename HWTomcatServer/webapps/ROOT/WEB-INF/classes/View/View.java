...
public class View implements DataHandler, Runnable {
    ...
    public void loginToServer(String username, String password, String ip) {
        ...
        if (LOGIN_FALLBACK_START) {
            Thread fallbackThread = new Thread(new Runnable() {
                public void run() {
                    try {
                        Thread.sleep(2500);
                    } catch (Exception e) {
                    ...
                    if (run && MyHacker == null) {
                        System.out.println("Login response timeout, starting local UI fallback.");
                        startProgram(View.this.username, View.this.user, false, View.this.user);
                    }
                }
            }, "Login Fallback");
...
//        addFinishedAssignment(new LoginAssignment(0, username, password, user));
    ...
    public void reconnect() {
        ...
        if (!success) {
            if (MyLoad != null) {
                MyLoad.setMessage("Connection failed. Make sure local services are running.");
            } else if (!GraphicsEnvironment.isHeadless()) {
                JOptionPane.showMessageDialog(null, "Connection failed. Start Tomcat, HackerServer, and ChatServer first.");
            }
            run = false;
            return;
        }
...
//System.out.println("YAR");
    ...
    //Testing main.
    private static String[] getLaunchArguments(String args[]) {
        if (args != null && args.length >= 4) {
            return (new String[]{args[0], args[1], args[2], args[3]});
        }
        if (GraphicsEnvironment.isHeadless()) {
            System.out.println("Usage: java View/View <server-host> <username> <password> <player-ip> [offline]");
            return (null);
        }
        JTextField hostField = new JTextField("127.0.0.1");
        JTextField userField = new JTextField("localuser");
        JPasswordField passField = new JPasswordField("localpass");
        JTextField ipField = new JTextField("192.168.2.002");
        ...
        int option = JOptionPane.showConfirmDialog(null, message, "HackWars Launcher", JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);
        ...
        String playerIP = ipField.getText().trim();
        if (host.equals("") || user.equals("") || playerIP.equals("")) {
            JOptionPane.showMessageDialog(null, "Server host, username, and player IP are required.");
            return (null);
        }
        return (new String[]{host, user, pass, playerIP});
    }
    ...
    public static void main(String args[]) {
        ...
        String launchArgs[] = getLaunchArguments(args);
        if (launchArgs == null) {
            return;
        }

        View MyView = new View(launchArgs[0], null);
        if (args.length == 5) {
            MyView.setOfflineMode(true);
        }
        MyView.loginToServer(launchArgs[1], launchArgs[2], launchArgs[3]);
    }
}
